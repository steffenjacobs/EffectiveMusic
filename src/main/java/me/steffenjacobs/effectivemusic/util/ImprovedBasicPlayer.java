 ...
 * 
 * modified by Steffen Jacobs
 * 
 * @see BasicPlayer
 */
@SuppressWarnings({ "rawtypes", "unchecked", "unused" })
public class ImprovedBasicPlayer implements BasicController, Runnable {
	...
	private final AtomicLong skipBytes = new AtomicLong(0);

	/**
	 * Constructs a Basic Player.
	 */
	public ImprovedBasicPlayer() {
		m_dataSource = null;
		...
		synchronized (m_audioInputStream) {
			...
			while ((nBytesRead != -1) && (m_status != STOPPED) && (m_status != SEEKING) && (m_status != UNKNOWN)) {
				if (m_status == PLAYING) {
					...
					try {
						if (skipBytes.get() > 0) {
							long bytes = skipBytes.get();

							System.out.println("size: " + m_audioInputStream.getFormat().getFrameSize());
							bytes *= m_audioInputStream.getFormat().getFrameSize();
							m_audioInputStream.read(new byte[(int) bytes], 0, (int) bytes);
							skipBytes.set(0);
						}
	...
	/**
	 * Skip bytes in the File inputstream. It will skip N frames matching to bytes,
	 * so it will never skip given bytes length exactly.
	 * 
	 * @param bytes
	...
	protected long skipBytes(long bytes) throws BasicPlayerException {
		...
		if (m_dataSource instanceof File && !(m_audioInputStream instanceof RingedAudioInputStream)) {
			...
			try {
				synchronized (m_audioInputStream) {
					System.out.println("sync-skipping");
	...
	protected long skipFrames(long bytes) throws BasicPlayerException {
		System.out.println("skipping natively...");
		long totalSkipped = 0;
		log.info("Bytes to skip : " + bytes);
		...
		try {
			synchronized (m_audioInputStream) {
				...
				notifyEvent(BasicPlayerEvent.SEEKING, getEncodedStreamPosition(), -1, null);
				initAudioInputStream();
				System.out.println("total bytess available: " + m_audioInputStream.available());
				if (m_audioInputStream != null) {
					// Loop until bytes are really skipped.
					int count = 0;
					// totalSkipped = Long.MAX_VALUE;
					while (totalSkipped < (bytes - SKIP_INACCURACY_SIZE)) {
						int available = m_audioInputStream.available();
						if (available <= (bytes - totalSkipped) && count <= 5) {
							skipped = m_audioInputStream.read(new byte[available], 0, available);
						} else {
							break;
							// skipped = m_audioInputStream.read(new byte[(int) (bytes - totalSkipped)]);
						}
						if (skipped == 0)
							break;
						totalSkipped = totalSkipped + skipped;
	 ...
	 * scale 0.0 <--> 100.0 Threshold Coef. : 1/2 to avoid saturation.
	 * 
	 * @param fGain:
	...
	public long getFramePosition() {
		if (m_line != null) {
			return m_encodedaudioInputStream.getFrameLength() + getEncodedStreamPosition();
		}
		return 0;
	}

	public long getFrameCount() {
		return m_encodedaudioInputStream.getFrameLength();
	}
	...
	/**
	 * @return true: position is before current position -> restart the track and
	 *         call method again
	 */
	public boolean setFramePosition(long position) throws BasicPlayerException {
		if (m_audioInputStream instanceof RingedAudioInputStream) {
			if (position > getFramePosition()) {
				long numFrames = position - getFramePosition();
				System.out.println("frame position to skip to: " + position);
				System.out.println("current position         : " + getFramePosition());
				System.out.println("total num of frames      : " + getFrameCount());
				System.out.println("frames to skip           : " + numFrames);
				// skipFrames(numFrames);
				skipBytes.set(numFrames / 2);
				System.out.println("new position             : " + getFramePosition());
				return false;
			} else {
				return true;
			}
